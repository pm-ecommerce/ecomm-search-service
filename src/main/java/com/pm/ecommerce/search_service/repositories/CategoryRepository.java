...
public interface CategoryRepository extends JpaRepository<Category, Integer> {
    ...
    @Query(value="Select c from Category c Join Product p on p.category.id=c.id Join Vendor v on p.vendor.id=v.id where p.status=:productStatus and v.status=:vendorStatus and c.isDeleted=false and c.parent IS NULL ORDER BY c.name")
    ...
    @Query(value="Select c from Category c Join Product p on p.category.id=c.id Join Vendor v on p.vendor.id=v.id where p.status=:productStatus and v.status=:vendorStatus and c.isDeleted=false and c.parent IS NOT NULL ORDER BY c.name")
