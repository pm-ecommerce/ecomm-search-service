...
//    List<Product> getProductsByFilter(@Param("name") String name, @Param("categoryId") Integer categoryId , @Param("highPrice") Double highPrice, @Param("lowPrice") Double lowPrice, Integer limit, Integer offset);
    ...
    @Query(value = "SELECT * FROM products p join categories c on c.id=p.category_id join vendors v on v.id=p.vendor_id WHERE p.name like %?1% and p.category_id =?2 and p.price BETWEEN ?4 and ?3 and p.status =?5 and c.is_deleted=?6 and v.status=?7 ORDER BY p.id desc", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM products p join categories c on c.id=p.category_id join vendors v on v.id=p.vendor_id WHERE p.category_id =?1 and p.price BETWEEN ?3 and ?2 and p.status =?4 and c.is_deleted=?5 and v.status=?6 ORDER BY p.id desc", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM products p join categories c on c.id=p.category_id join vendors v on v.id=p.vendor_id WHERE p.name like %?1% and p.price BETWEEN ?3 and ?2 and p.status =?4 and c.is_deleted=?5 and v.status=?6 ORDER BY p.id desc", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM products p join categories c on c.id=p.category_id join vendors v on v.id=p.vendor_id WHERE p.price BETWEEN ?2 and ?1 and p.status =?3 and c.is_deleted=?4 and v.status=?5 ORDER BY p.id desc", nativeQuery = true)
